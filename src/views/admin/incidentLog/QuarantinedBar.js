...
import {
  ...
  export default function QuarantinedBar(props) {
    const { ...rest } = props;

    const barChartDataQuarantined = [
      {
        name: "Finance",
    ...
    const barChartOptionsQuarantined = {
      chart: {
        stacked: true,
        toolbar: {
          show: false,
    ...
    return (
      <Card align='center' direction='column' w='100%' {...rest} boxShadow={cardShadow}>
        ...
        <Box h='240px' mt='auto'>
          <BarChart
            chartData={barChartDataQuarantined}
            chartOptions={barChartOptionsQuarantined}
          />
        </Box>
      </Card>
    );
